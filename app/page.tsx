...
export default function Home() {
  ...
  const handleInputChange = (day: keyof typeof counts, step: number, field: string, value: string) => {
    ...
    if (errors[day][step] && errors[day][step][field as keyof (typeof errors)[day][0]]) {
      setErrors((prev) => {
  ...
  const renderSuccessScreen = (day: keyof typeof counts) => {
    ...
    return (
      ...
      >
        ...
        <motion.p className="text-xl sm:text-2xl mb-6 sm:mb-8 max-w-md leading-relaxed" variants={textVariants}>
          thank you so much for deciding to spend your {dayText} at{" "}
        ...
        <motion.p className="text-xl sm:text-2xl mb-10 sm:mb-16 max-w-md leading-relaxed" variants={textVariants}>
          here's your personal invitation card <br />- it's yours to keep forever :)
        </motion.p>
        ...
        <motion.div
          ...
          style={{
            boxShadow: "0 10px 25px rgba(84, 42, 49, 0.2), 0 6px 12px rgba(84, 42, 49, 0.15)",
        ...
        >
          ...
          <div className="p-6 pb-12">
            ...
            {/* Name */}
            <h2 className="text-2xl font-medium text-center mb-8 italic">{fullName}</h2>
            ...
            <div className="text-center text-sm mb-6">past studies, chicago</div>
          </div>
      ...
      <div className="w-full max-w-[90%] sm:max-w-md md:max-w-lg">
        ...
        {submitted.thursday ? (
          ...
          <>
            ...
            >
              <h2
                className={`text-base sm:text-lg text-[#949494] mb-6 blurred-text ${startBlurAnimation ? "unblurred" : ""}`}
              >
                spring is here!
              </h2>
              <p className="text-base sm:text-lg leading-relaxed mb-8">
                <span className={`text-[#949494] blurred-text ${startBlurAnimation ? "unblurred" : ""}`}>
                ...
                <span className={`text-[#949494] blurred-text ${startBlurAnimation ? "unblurred" : ""}`}>
                  , would like to extend a warm invitation to the beautiful people of chicago to come celebrate this
                  fantastic weather at{" "}
                </span>
                <span className="text-[#542a31] italic">past studies</span>
                <span className={`text-[#949494] blurred-text ${startBlurAnimation ? "unblurred" : ""}`}>
                  {" "}
                  over some decadent iced coffee and delicious ceremonial matcha.
                </span>
              </p>
            </div>
...
function DateSelector({ day, date, count, onIncrement, onDecrement }: DateSelectorProps) {
  ...
  return (
    <div className="flex flex-col sm:flex-row sm:items-center justify-between py-2 gap-3 sm:gap-0">
      <div>
        <span className="text-[#542a31] text-xl sm:text-2xl font-medium">{day}</span>{" "}
        <span className="text-black text-xl sm:text-2xl font-normal">{date}</span>
      </div>
